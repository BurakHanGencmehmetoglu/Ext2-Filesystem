...
struct ext2_super_block {
    uint32_t s_inode_count; /* Total number of inodes in the fs */
    uint32_t s_block_count; /* Total number of blocks in the fs */
    uint32_t s_reserved_block_count; /* Number of blocks reserved for root */
    uint32_t s_free_block_count; /* Number of free blocks */
    uint32_t s_free_inode_count; /* Number of free inodes */
    uint32_t s_first_data_block; /* The first data block number */
    uint32_t s_log_block_size; /* 2^(10 + this value) gives the block size */
    uint32_t s_log_fragment_size; /* Same for fragments (we won't use fragments) */
    uint32_t s_blocks_per_group; /* Number of blocks for each block group (last group can have fewer) */
    uint32_t s_fragments_per_group; /* Same for fragments */
    uint32_t s_inodes_per_group; /* Number of inodes for each block group (last group can have fewer) */
    uint32_t s_mount_time; /* Mounting and modification metadata, many less important fields */
    uint32_t s_write_time;
    uint16_t s_mount_count;
    uint16_t s_max_mount_count;
    uint16_t s_magic; /* Magic field, should be EXT2_SUPER_MAGIC */
    uint16_t s_state;
    uint16_t s_errors;
    ...
    uint16_t s_default_uid;
    uint16_t s_default_gid;
    uint32_t s_first_inode; /* First non-reserved inode in the filesystem */
    uint16_t s_inode_size; /* Size of each inode */
